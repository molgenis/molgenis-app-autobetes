...
##### BEFORE #####
touch $PBS_O_WORKDIR/s26_VcfToTable_Test_DNA.out
source $WORKDIR/tools/scripts/import.sh
before="$(date +%s)"
echo "Begin job s26_VcfToTable_Test_DNA at $(date)" >> $PBS_O_WORKDIR/RUNTIME.log
...
getFile $WORKDIR/tmp//demo/output//Test_DNA.snps.final.vcf

getFile $WORKDIR/tools/GATK-1.0.5069/Sting/python/vcf2table.py
getFile $WORKDIR/tools/scripts/snp_annotation_stats.pl

module load Python/2.7.3

####Transform VCF file into tabular file####
python $WORKDIR/tools/GATK-1.0.5069/Sting/python/vcf2table.py \
$WORKDIR/tmp//demo/output//Test_DNA.snps.final.vcf \
...
sumGLbyD,\
FORMAT,Test_DNA \
-o $WORKDIR/tmp//demo/output//Test_DNA.snps.final.vcf.table

# get SNP statistics
perl $WORKDIR/tools/scripts/snp_annotation_stats.pl \
-vcf_table $WORKDIR/tmp//demo/output//Test_DNA.snps.final.vcf.table \
-typefile $WORKDIR/tmp//demo/output//Test_DNA.snps.final.type.txt \
-classfile $WORKDIR/tmp//demo/output//Test_DNA.snps.final.class.txt \
-impactfile $WORKDIR/tmp//demo/output//Test_DNA.snps.final.impact.txt \
...
putFile $WORKDIR/tmp//demo/output//Test_DNA.snps.final.vcf.table
putFile $WORKDIR/tmp//demo/output//Test_DNA.snps.final.type.txt
putFile $WORKDIR/tmp//demo/output//Test_DNA.snps.final.class.txt
putFile $WORKDIR/tmp//demo/output//Test_DNA.snps.final.impact.txt

###### AFTER ######
after="$(date +%s)"
elapsed_seconds="$(expr $after - $before)"
echo Completed s26_VcfToTable_Test_DNA at $(date) in $elapsed_seconds seconds >> $PBS_O_WORKDIR/RUNTIME.log
touch $PBS_O_WORKDIR/s26_VcfToTable_Test_DNA.finished
######## END ########
