...
public class importCommandLine
{
	 ...
	 * Save file in DB
	 */
	private static void fileToDB(Database db, File f) throws IOException, ParseException, DatabaseException, DataFormatException
	{
		...
		CsvReader reader = new CsvStringReader(fsplit.get(BODY));
		...
		checkAdd(db, asIdentifiableList(exerciseListFile));
		System.out.println(">> Trying to add carbs...");
		checkAdd(db, asIdentifiableList(carbListFile));
		System.out.println(">> Trying to add bgsensor...");
		checkAdd(db, asIdentifiableList(bgsensorListFile));
		System.out.println(">> Trying to add bolus...");
		checkAdd(db, asIdentifiableList(bolusListFile));
		System.out.println(">> Trying to add basal as programmed...");
		checkAdd(db, asIdentifiableList(basalProgrammedListFile));
		System.out.println(">> Trying to add basal temp change...");
		checkAdd(db, asIdentifiableList(basalTempListFile));
		...
		checkAdd(db, asIdentifiableList(basalList));
		...
		checkAdd(db, asIdentifiableList(basalAsReleased));
	...
	{
		Iterator<Identifiable> it = ((List<Identifiable>) o).iterator();
	...
	@SuppressWarnings("unchecked")
	private static List<Identifiable> asIdentifiableList(Object o)
	{
		return (List<Identifiable>) o;
	}

	/**
	 * Check consistency with DB, and add if non-existent
	 */
	private static void checkAdd(Database db, List<Identifiable> itemListFile) throws DatabaseException
	{
		...
		List<Identifiable> addList = new ArrayList<Identifiable>();

		// get map unixtimes -> entity (File)
		LinkedHashMap<Long, Identifiable> itemMapFile = getUnixtimesMap(itemListFile);
		...
		LinkedHashMap<Long, Identifiable> itemMapDB = getUnixtimesMap(q.find());
		...
		while (it.hasNext())
		{
			...
			Identifiable itemDb = itemMapDB.get(unixtime);
			Identifiable itemFile = itemMapFile.get(unixtime);
			...
			// check consistency
			{
				...
				if (!consistent)
				{
					System.err.println(">> Inconsistency identifiable in File with DB");
	...
	private static boolean summertimeIssue(Identifiable itemFile)
	{
	...
	/**
	 * Return map<unixtime, identifiable>
	 */
	@SuppressWarnings("unchecked")
	private static LinkedHashMap<Long, Identifiable> getUnixtimesMap(Object olst)
	{
		LinkedHashMap<Long, Identifiable> map = new LinkedHashMap<Long, Identifiable>();

		List<Identifiable> lst = (List<Identifiable>) olst;

		Iterator<Identifiable> it = lst.iterator();
		while (it.hasNext())
		{
			Identifiable i = it.next();
			...
			{
				System.err.println(">> Unixtime " + i.getUnixtime() + " is twice in identifiable-list that you want to import...");
				System.err.println(">> System exit with status 1.");
				System.exit(1);
			} else
			{
	...
	private static void setDateTime(Identifiable e, Tuple tuple)
	{
